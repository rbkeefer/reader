...
int main(int argc, char** argv)
{
  ...
  regionImg = displayBoundaries(regionImg, foundBoundingRectList, cvScalar(0, 0, 255), 2);
...
void expandBoundingRectList(list<BoundingRectangle*> inRectangleList)
{
  ...
  for (iter=inRectangleList.begin(); iter!=inRectangleList.end(); iter++)
  {
    ...
    rectangle->bottom = (rectangle->bottom * BOX_SIZE * timesReduced + BOX_SIZE);
    rectangle->left = rectangle->left * BOX_SIZE* timesReduced;
    rectangle->right = (rectangle->right * BOX_SIZE * timesReduced + BOX_SIZE);
  }
}
