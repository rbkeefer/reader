...
bool isColoredRegion(int inColumn,
                     int inRow,
                     IplImage* inImage,
                     int inRegionSize,
                     int inColor);

bool isColoredColumn(int inColumn,
                     int inStartRow,
                     int inEndRow,
                     IplImage* inImage,
                     int inColor);

bool isColoredRow(int inColumn,
                  int inStartRow,
                  int inEndRow,
                  IplImage* inImage,
                  int inColor);
...
{
  return isColoredRegion(inColumn, inRow, inImage, inRegionSize, BLACK);
}

bool isWhiteRegion(int inColumn, int inRow, IplImage* inImage, int inRegionSize)
{
  return isColoredRegion(inColumn, inRow, inImage, inRegionSize, WHITE);
}
...
bool isColoredRegion(int inColumn, int inRow, IplImage* inImage, int inRegionSize, int inColor)
{
  ...
  for( rowCounter=0; rowCounter<inRegionSize; rowCounter++ )
  {
    ...
    {
      if(inImageData[(inImageStep*(inRegionSize*inRow+columnCounter)) + 
                     (inRegionSize*inColumn+rowCounter)] == inColor )
      {
        return true;
      }
    }
...
{
  return isColoredColumn(inColumn, inStartRow, inEndRow, inImage, BLACK);
}

bool isWhiteColumn(int inColumn, int inStartRow, int inEndRow, IplImage* inImage)
{
  return isColoredColumn(inColumn, inStartRow, inEndRow, inImage, WHITE);
}
...
{
  return isColoredRow(inRow, inStartColumn, inEndColumn, inImage, BLACK);
}

bool isWhiteRow(int inRow, int inStartColumn, int inEndColumn, IplImage* inImage)
{
  return isColoredRow(inRow, inStartColumn, inEndColumn, inImage, WHITE);
}
