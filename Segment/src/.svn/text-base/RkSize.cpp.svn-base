...
void displayLines(IplImage* inImpImage, int inWidth, int inMultiplier, list<RkColumn*> inColumnList, list<RkRow*> inRowList);
...
IplImage* reduceImage(IplImage* inImage, int inWidth, int inDemoMode)
{
  ...
  list<RkColumn*> RkColumnList = findColumns(tmpImage, MIN_HEIGHT, WHITE);
  ...
  tmpImage = cleanImageLines(tmpImage, expandedRkColumnList, expandedRkRowList, WHITE);

  if (inDemoMode)
	displayLines(tmpImage, origWidth, origWidth/inWidth, expandedRkColumnList, expandedRkRowList);

  return tmpImage;
}

void displayLines(IplImage* inImage, int inWidth, int inMultiplier, list<RkColumn*> inColumnList, list<RkRow*> inRowList)
{
  ...
  for (columnIter=inColumnList.begin(); columnIter!=inColumnList.end(); columnIter++)
  {
    ...
    cvLine(tmpImage, topPoint, bottomPoint, cvScalar(0, 0, 255), 2);
  }
  ...
  for (rowIter=inRowList.begin(); rowIter!=inRowList.end(); rowIter++)
  {
    ...
    cvLine(tmpImage, leftPoint, rightPoint, cvScalar(255, 0, 0), 2);
  }
