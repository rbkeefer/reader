...
void displayLines(IplImage* inImpImage, list<RkColumn*> inColumnList, list<RkRow*> inRowList);
...
IplImage* cleanupImage(IplImage* inImage)
{
  ...
  // displayLines(tmpImage, expandedRkColumnList, expandedRkRowList);

return tmpImage;
}

void displayLines(IplImage* inImage, list<RkColumn*> inColumnList, list<RkRow*> inRowList)
{
	IplImage* tmpImage = cvCreateImage(cvSize(inImage->width, inImage->height), IPL_DEPTH_8U, 3);
	cvMerge(inImage, inImage, inImage, NULL, tmpImage);
	...
	{
	  column = (*columnIter);
	  if (column->bottom >= height) column->bottom = height-1;
	  if (column->column >= width) column->column = width-1;
	  cvLine(tmpImage, column->topPoint(), column->bottomPoint(), cvScalar(0, 0, 255), 1);
	}
	...
	{
		row = (*rowIter);
	  if (row->right >= width) row->right = width-1;
	  if (row->row >= height) row->row = height-1;
	  cvLine(tmpImage, row->leftPoint(), row->rightPoint(), cvScalar(255, 0, 0), 1);
	}
