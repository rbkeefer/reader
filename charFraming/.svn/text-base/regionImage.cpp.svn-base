...
int getValidPosition(CvPoint inCurrent, int inPosition)
{
  ...
  if ((inPosition == 3) && (inCurrent.x == imageWidth-1)) return getValidPosition(inCurrent, 6);
  if ((inPosition == 3) && (inCurrent.y == 0)) return getValidPosition(inCurrent, 4);

  if ((inPosition == 4) && (inCurrent.x == imageWidth-1)) return getValidPosition(inCurrent, 6);

  if ((inPosition == 5) && (inCurrent.x == imageWidth-1)) return getValidPosition(inCurrent, 6);
  if ((inPosition == 5) && (inCurrent.y == imageHeight-1)) return getValidPosition(inCurrent, 0);

  if ((inPosition == 6) && (inCurrent.y == imageHeight-1)) return getValidPosition(inCurrent, 0);

  if ((inPosition == 7) && (inCurrent.x == 0)) return getValidPosition(inCurrent, 2);
  if ((inPosition == 7) && (inCurrent.y == imageHeight-1)) return getValidPosition(inCurrent, 0);
...
int findNextPosition(CvPoint inCurrent, int inPosition, IplImage* inImage, int inColor)
{
  ...
  while (endN != (nPosition%8))
  {
    ...
    if (! samePoint(inCurrent, neighbor))
    {
      pixel = (neighbor.y * imageWidth) + neighbor.x;

      // printf("              curN:[%d]   Neighbor:[%d, %d]\n", nPosition%8, neighbor.x, neighbor.y);
...
BoundingRectangle* regionBoundary(int inColumn, int inRow, IplImage* inImage, int inColor)
{
  ...
  while (! samePoint(current, start))
  {
    ...
    if (counter++ > imageWidth*imageHeight) break;
